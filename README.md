# ПОДВОЖУ ИТОГ - ОНО ТОГО НЕ СТОИТ ( МОЙ СИНТАКСИС MD ДОКУМЕНТА 
# ОКОЛОНУЛЕВОЙ И ЛУЧШЕ Я БУДУ ПИСАТЬ ЗАМЕТКИ НА ЛИСТИКЕ И РАЗБИРАТЬ 
# ОДНОВРЕМЕННО КОД В IDEA ЧЕМ ТРАТИТЬ СВОЁ ВРЕМЯ НА ЭТОТ ДЕБИЛИЗМ. 
# НАКИПЕЛО=))

# ПОТОКИ ВВОДА-ВЫВОДА ГИЛБЕРТ ШИЛДТ НАПИСАЛ КАК &#^@*! ОДНИМ СЛОВОМ. 
# БУДУ ПЕРЕЧИТЫВАТЬ)))))))))) ЗАЧЕТ ОКОЛО 20Х ЧИСЛАХ))) SCM - НЕ 
# ЧИТАЛОСЬ, БЛИНОВ - ЗАКРЫЛСЯ НА 20Й, ЭККЕЛЬ - КАЙФУЕТ, ШИЛД 400/640=)) 
ОДНИМ СЛОВОМ - PI1@$*!

# Что читаю сейчас:

Шилдт-Герберт-Java-8-руководство-для-начинающих-6-е-издание-2015 - currentProgress ( 397/641 )
